...
#_________________Donnée__________________________#

HEIGH=490
WIDTH=775

velo=28 # corespond a la velossiter
nbleu=(19,43,133)
bleux=(0,0,255)
black=(0,0,0)
blank=(255,255,255)
...
#_____________________partie visualisateur______________#
class Carre(pygame.sprite.Sprite):
        def __init__(self, x, y, t):
                pygame.sprite.Sprite.__init__(self)
                self.image = pygame.Surface((500,50))
                pygame.draw.rect(self.image,nbleu, pygame.Rect(0,0,500,50))
                self.rect = self.image.get_rect()
                self.rect.center = (x, y)
                police = pygame.font.SysFont('Helvetica', 24, bold=True)
                texte = police.render(t, True,blank)
                self.image.blit(texte,(10,10))
...
class Ffenetre():
      ...
      def __init__(self,x,y,t) :
          self.fin=pygame.sprite.Group()
          self.fin.add(Carre(x,y,t))
          

      def draw(self, screen):
        self.fin.update()
        self.fin.draw(screen)    

 

class Cpryte(pygame.sprite.Sprite):

    def __init__(self,t,z,c):
        # !!! t a deux apramétre x et y t=(x,y) pour les coordoner !!!
        
        pygame.sprite.Sprite.__init__(self)
        self.image = pygame.Surface((25,25))
        pygame.draw.circle(self.image,c,(13,13),10)
        self.rect= self.image.get_rect() 
        self.rect.center=t
...
class Armie():
    ...
    def grile_from_px(self,x,y):
        i=183+(y*velo)
        j=35+(x*velo)
        return(i,j)

    def __init__(self,army,c):
        self.all = pygame.sprite.Group()
        
        for i in army.unites:
            t=self.grile_from_px(i.x,i.y)
            self.all.add(Cpryte(t,i,c))
...
class Deplacement (): 
    ...
    def __init__(self,groupe1,groupe2):
        self.p=Armie(groupe1,red)
        self.p2=Armie(groupe2,bleux)
        
                 
    def draw(self, screen,groupe1,groupe2):
        self.p.all.update()
        self.p.all.draw(screen)
        self.p2.all.update()
        self.p2.all.draw(screen)
        
if __name__ == '__main__':
    pygame.init()
    pygame.display.set_caption("Wargame Match")
    screen= pygame.display.set_mode((WIDTH,HEIGH))
    background= pygame.image.load("img/bg.jpg")
    arial=pygame.font.SysFont("arial",10,True)
...
class Simulateur :
    ...
    def __init__(self,size):
        self.sprite_army_1=0
        self.sprite_army_2=0
        self.conteur=0
        self.map=[]
        self.biomes=[] #inutilisé
        self.size=size
        self.viewer_win=0
        self.viewer_name_1=0
        self.viewer_name_2=0
    ...
    def set_army1(self,army):
        ...
        for unit in army.unites:
            if c > self.size-1:
                ...
                self.set_unity(unit)
                c+=1
#______________affichage initiale des armée__________#
                
        self.sprite_army_1=army
        self.viewer_name_1=army.name
...
#______________affichage initiale des armée___________#
                
        self.sprite_army_2=army
        self.viewer_name_2=army.name  
...
#____________________partie viewer_________________#

            self.conteur+=1
            
            if len(army.unites)==1 or len(army2.unites)==1:
                self.conteur=0
                background= pygame.image.load("img/bg.jpg")
                arial=pygame.font.SysFont("arial",12,True)
        
                text_name1=arial.render(self.viewer_name_1,False,blank)
                background.blit(text_name1,(coo_1,coo_costente))

                text_name2=arial.render(self.viewer_name_2,False,blank)
                background.blit(text_name2,(coo_2,coo_costente))    
                
                screen.blit(background,(0,0))
                 
                move= Deplacement (self.sprite_army_1,self.sprite_army_2)
                move.draw(screen,self.sprite_army_1,self.sprite_army_2)
                pygame.display.flip()
            else:
...
#___________________visu____________________#
   ...
   #________affichage fenetre victoire_______#
        
        fin=Ffenetre(377,245,self.viewer_win)
        fin.draw(screen)
        pygame.display.flip()
        
#_________________visu______________________#
        army_defense.unites = army_defense.unites_copy                  
    ...
    def win_army(self,army1,army2):
        if army1.unites and army2.unites:
            ...
            if not army1.unites:
                ...
                army2.nb_match+=1
                army1.nb_match+=1
                self.viewer_win=army2.name
                return army2
            else:
                army1.nb_victory+=1
                army1.nb_match+=1
                army2.nb_match+=1
                self.viewer_win=army1.name
                return army1
